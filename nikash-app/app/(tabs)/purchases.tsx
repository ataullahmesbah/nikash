...
export default function PurchasesScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, stale: isStale } = await loadWithCache(
      `nikash:purchases-list:${profile.company_id}:${range.from}:${range.to}`,
      async () => {
        const { data, error } = await supabase
          .from("purchases")
          .select(
            "id, invoice_no, entry_date, total, due, status, parties:supplier_id(name)"
          )
  ...
  const visible = useMemo(() => {
    ...
    return purchases.filter((s) => {
      ...
      if (!q) return true;
      const party = (s.parties?.name ?? "").toLowerCase();
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.bg }}>
      <View style={styles.top}>
        <PrimaryButton title="+ নতুন ক্রয়" onPress={() => router.push("/purchases/new")} />
        ...
        <View style={{ marginTop: 10 }}>
          <SearchBar value={search} onChange={setSearch} placeholder="চালান নম্বর বা সরবরাহকারীর নাম..." />
        </View>
        ...
        <Card style={styles.summary}>
          ...
          <SummaryCell label="মোট ক্রয়" value={taka(totals.amount)} />
          <View style={styles.divider} />
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              icon="🧾"
              title={search || filter !== "all" ? "কিছু পাওয়া যায়নি" : "এই সময়ে কোনো ক্রয় নেই"}
              message={
                search || filter !== "all"
                  ? "খোঁজা বা ফিল্টার বদলে দেখুন"
                  : "উপরের তারিখ বদলে অন্য সময়ের হিসাব দেখুন।"
              }
              action={{ label: "+ নতুন ক্রয়", onPress: () => router.push("/purchases/new") }}
            />
          }
          renderItem={({ item }) => {
            const st = STATUS[item.status];
            return (
              <Card onPress={() => router.push(`/purchases/${item.id}`)} style={styles.row}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.name} numberOfLines={1}>
                    {item.parties?.name ?? "নগদ ক্রয়"}
                  </Text>
